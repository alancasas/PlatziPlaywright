...
test('Realizar una busqueda que no tenga resultados box', async ({ page }) => {
  ...
  await expect(page.locator('.DocSearch-NoResults p')).toContainText('No results for "hascontent"');
...
test('Limpiar el input de busqueda', async ({ page }) => {
  ...
  await expect(page.locator('.DocSearch-NoResults p')).toContainText('"somerandomtext"');
